// config.js
export const config = {
    UI: "https://invidious.nerdvpn.de/", // This loads the frontend for watch together, displays all videos
    sponsorBlock: "https://sponsor.ajay.app/", // Blocks all sponsors
    dearrow: "https://dearrow.ajay.app/", // Shows nonclick titles
    API: "https://invidious.kavin.rocks/api/v1/", // Plays videos & fetches them
   // Proxy: "https://cors-anywhere.herokuapp.com/"  // Bypass CORS errors and resolves them
};
